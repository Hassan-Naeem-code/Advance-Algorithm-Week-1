...
def create_features_and_scale(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # `balance` is not present in this dataset; use `duration`
    # (call length). Clip at 0 to avoid negative values before log1p.
    df["duration_log"] = np.log1p(
        df["duration"].clip(lower=0)
    )

    # 3) Campaign to previous contacts ratio (small engineered signal)
    # `previous` is number of contacts performed before this campaign for the
    # client. Use (previous + 1) in denominator to avoid division by zero.
    df["campaign_prev_ratio"] = (
    ...
    has_nans = df[num_cols].isnull().any().any()
    assert not has_nans, (
        "Numeric columns contain NaNs before scaling"
    )
